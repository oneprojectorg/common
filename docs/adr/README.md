...
An ADR records one architecture decision: the problem, the options, and the
choice. Keep it to one page. We use [MADR](https://adr.github.io/madr/), for the
reasons in [ADR 0001](./0001-record-architecture-decisions.md).

[`adr-template.md`](./adr-template.md) is MADR's
[minimal template](https://github.com/adr/madr/blob/develop/template/adr-template-minimal.md)
plus frontmatter, where `status` and `date` are required. Upstream also publishes
a longer template. We do not use it.
