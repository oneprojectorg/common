...
export interface ProfilePopupProps {
  ...
  compact?: boolean;
  onContactClick?: (profile: ProfileData) => void;
  onViewProfileClick?: (profile: ProfileData) => void;
}
...
export function ProfilePopup({ 
  ...
  return (
    <div className={`bg-white ${compact ? 'p-3 min-w-[180px]' : 'p-4 min-w-[240px]'} max-w-[320px]`}>
      ...
      <div className="flex items-start gap-3 mb-3">
        {profile.avatar ? (
          ...
          <div className={`${compact ? 'w-8 h-8' : 'w-10 h-10'} rounded-full flex items-center justify-center flex-shrink-0 ${
        ...
        <div className="flex-1 min-w-0">
          <h3 className={`font-semibold ${compact ? 'text-sm' : 'text-base'} text-neutral-900 mb-1 truncate`}>
            {profile.name}
          </h3>
      ...
      {/* Description */}
      <p className={`${compact ? 'text-xs' : 'text-sm'} text-neutral-600 mb-3 line-clamp-2`}>
        {profile.description}
      </p>
      ...
      {showCoordinates && (
        <div className="pt-2 border-t border-neutral-100">
          <div className="text-xs text-neutral-400">
            Lat: {profile.latitude.toFixed(4)}, Lng: {profile.longitude.toFixed(4)}
          </div>
        </div>
      )}
    </div>
