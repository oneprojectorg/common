...
const badgeVariants = cva(
  'group/badge relative isolate inline-flex h-5 w-fit shrink-0 items-center justify-center gap-1 overflow-hidden rounded-sm border border-transparent px-2 py-0.5 font-sans text-xs font-strong whitespace-nowrap transition-all after:pointer-events-none after:absolute after:inset-0 after:-z-10 after:rounded-[inherit] after:transition-colors focus-visible:border-ring focus-visible:ring-[3px] focus-visible:ring-ring/50 has-data-[icon=inline-end]:pe-1.5 has-data-[icon=inline-start]:ps-1.5 aria-invalid:border-destructive aria-invalid:ring-destructive/20 dark:aria-invalid:ring-destructive/40 [&>svg]:pointer-events-none [&>svg]:size-3!',
  {
    variants: {
      variant: {
        default:
        ...
        // Dashed, for something that isn't real yet — an address invited but not
        // yet an account, a slot not yet filled. The same vocabulary the draft
        // scaffolding uses, so "dashed" means one thing across the product.
        pending:
          'border-dashed border-input bg-background text-muted-foreground [&:is(a,button)]:hover:bg-muted',
