...
- `sheet.tsx` — added the `fullScreenOnMobile` prop, which restates the inline
  sides' border and translate at the same variant chain so `cn` drops the stock
  ones, then restores both at `sm`. A `max-sm:` override cannot do this:
  Tailwind orders variants in the stylesheet, so it loses regardless of class
  order.
