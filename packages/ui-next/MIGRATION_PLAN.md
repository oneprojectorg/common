...
5. **MultiSelectComboBox.** Deferred from Tier 3a/3b. 9 consumers stay on `@op/ui/MultiSelectComboBox` (RAC-based) until Tier 3.5 builds the shadcn-equivalent custom component on top of `@base-ui/react` Combobox with: async creation (`isNewValue`), level/hasChildren hierarchy, TagGroup for selected items, optional local search. `apps/app/src/components/form/utils.tsx` keeps MultiSelectComboBox import from `@op/ui` until then.

6. **Vanilla shadcn for trivial components.** Tier 3b will codemod Checkbox (7 sites), RadioGroup (6), ToggleButton→Switch (13), ComboBox (1) to pure shadcn primitives — no compat wrappers. Prop renames + JSX restructure (label as sibling, value/onValueChange instead of onChange, etc.).
