...
export const createInstanceFromTemplate = async ({
  ...
  // Checked even for the session default: `currentProfileId` is a last-viewed
  // pointer, so a revoked org admin would otherwise keep stewarding to it.
  const stewardProfileId =
    requestedStewardProfileId ?? dbUser.currentProfileId ?? ownerProfileId;

  await assertCanStewardToProfile({ user, stewardProfileId, ownerProfileId });
