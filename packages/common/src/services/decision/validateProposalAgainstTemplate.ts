...
/**
 * The proposal `budget` field appears in two stored shapes depending on the
 * template:
 *  - `{ type: 'number', maximum: X }` — generated by CreateDecisionProcessModal
 *  - `{ type: 'object', x-format: 'money', properties: { amount, currency } }`
 *    — the canonical seeded shape
 *
 * `parseProposalData` always normalizes the stored value to `{ amount, currency }`,
 * so we need to coerce it back to a plain number whenever the template expects
 * one. Otherwise AJV produces "must be a number" before the `maximum` keyword
 * fires and the cap goes unenforced.
 */
function coerceBudgetForTemplate(
  ...
  if (parsed.collaborationDocId) {
    ...
    const validationData = {
      ...
      ...(storedProposalData.budget !== undefined
        ? { budget: coerceBudgetForTemplate(parsed.budget, proposalTemplate) }
        : {}),
