...
test.describe('Proposal View', () => {
  ...
  test('renders proposal view for a legacy cowop state-based instance', async ({
    authenticatedPage,
    org,
  }) => {
    // Reproduces the regression introduced in #1001: the proposal page added
    // a getInstance suspense query, which parses through the strict
    // processInstanceWithSchemaEncoder. Real cowop production instances are
    // stored with the pre-v2 state-based shape (processSchema.states,
    // instanceData.phases[].stateId) and are rejected by that encoder, so
    // the page 500s. See
    // services/api/src/routers/decision/instances/listLegacyInstances.test.ts
    // for the matching server-side legacy fixture.

    // 1. Process row generated by the real cowop schema function — this is
    //    the exact shape stored for live cowop instances on prod (states[],
    //    no top-level id/version).
    const cowopLegacySchema = cowopSchema({
      processName: 'COWOP Democratic Budgeting',
      description: 'Legacy cowop process for e2e regression test',
      totalBudget: 100000,
      budgetCapAmount: 100000,
      requireBudget: true,
      categories: [
      ...
      ideaCollectionPhase: {
        ideaCollectionOpen: '2025-01-01',
        ideaCollectionClose: '2025-02-01',
      },
      proposalSubmissionPhase: {
        submissionsOpen: '2025-02-02',
        submissionsClose: '2025-03-01',
      },
    });
    ...
    const [legacyProcess] = await db
      .insert(decisionProcesses)
      .values({
        name: `COWOP Legacy Process ${randomUUID().slice(0, 8)}`,
        description: 'Legacy cowop state-based process for e2e regression test',
    ...
    // 3. Instance with legacy-shape instanceData matching real cowop prod:
    //    - phases[].stateId (not phaseId)
    //    - plannedStartDate / plannedEndDate (not startDate / endDate)
    ...
    if (!processInstance) {
      throw new Error('Failed to create legacy cowop process instance');
    }

    // 4. Grant admin access on the decision profile
    const [profileUser] = await db
      .insert(profileUsers)
      .values({
        profileId: instanceProfile.id,
    ...
    // 5. Create a proposal under the legacy cowop instance
    const proposal = await createProposal({
      processInstanceId: processInstance.id,
      submittedByProfileId: org.organizationProfile.id,
      authUserId: org.adminUser.authUserId,
      email: org.adminUser.email,
      proposalData: {
        title: 'Legacy COWOP Proposal',
        description:
          '<p>Proposal under a legacy cowop state-based instance.</p>',
      },
    });
    ...
    await expect(
      authenticatedPage.getByRole('heading', { name: 'Legacy COWOP Proposal' }),
    ).toBeVisible({ timeout: 30_000 });
  });
