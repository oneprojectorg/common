...
test.describe('Proposal View', () => {
  ...
  test('renders proposal view for a legacy cowop state-based instance', async ({
    ...
    // Regression for #1001: legacy cowop instances are stored with
    // states[] / phases[].stateId / plannedStartDate, which the v2
    // encoder rejects. Pre-fix, both proposal routes 500'd on this data.
    const cowopLegacySchema = cowopSchema({
      processName: 'COWOP Democratic Budgeting',
