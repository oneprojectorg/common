...
If you intentionally rewrite the baseline (e.g. a refactor that legitimately moves many fingerprints), regenerate from a fresh local scan:

```bash
pnpm build:e2e          # ensure local build matches latest source
pnpm start:e2e &
pnpm a11y:seed          # overwrites known-violations.json from the current scan
git diff tests/e2e/a11y-baseline/known-violations.json   # sanity-check
```

`pnpm a11y:seed` is forbidden in CI — seeding bypasses comparison and would let regressions land green.
...
Stale e2e build artifacts can produce a different render than CI. If `pnpm a11y:baseline` shows the same new/resolved diff on a route that hasn't actually changed, refresh the build before re-running:
