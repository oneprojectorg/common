...
describe('resources.list / listByCollection', () => {
  ...
  it('listByCollection rejects an outsider with NotFoundError (existence is hidden)', async ({
    task,
    onTestFinished,
  }) => {
...
describe('resources.update', () => {
  ...
  it('rejects a non-admin member from updating a resource (NotFoundError to hide existence)', async ({
    task,
    onTestFinished,
  }) => {
