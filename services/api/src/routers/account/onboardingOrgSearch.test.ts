...
describe.concurrent('Onboarding Organization Search', () => {
  ...
  describe.concurrent('US-009: individual user without org can skip to app', () => {
    ...
    it('creates no join requests when user skips org selection', async ({
      ...
      expect(Array.isArray(matchingOrgs.items)).toBe(true);
  ...
  describe.concurrent('US-010: individual user can search, select orgs, and submit join requests', () => {
    it('returns matching organizations from search', async ({
      ...
      const found = result.items.find(({ org }) => org.id === organization.id);
    ...
    it('flags orgs the searcher already belongs to with isMember', async ({
      ...
      const found = new Map(result.items.map((entry) => [entry.org.id, entry]));
  ...
  describe.concurrent('US-011: user with domain-matched org sees pre-selected org and can auto-join', () => {
    it('returns domain-matched organizations for user with matching email', async ({
      ...
      const matchedOrg = result.items.find(
    ...
    it('excludes domain-matched orgs the user is already a member of', async ({
      ...
      const ids = result.items.map(({ org }) => org.id);
    ...
    it('allows domain-matched user to auto-join via organization.join', async ({
      ...
      const matchedOrg = matchingOrgs.items.find(
        ({ org }) => org.id === organization.id,
      );
      expect(matchedOrg).toBeDefined();
    ...
    it('returns whereWeWork location data for domain-matched organizations', async ({
      ...
      onTestFinished(async () => {
        await db
          ...
          .where(eq(organizationsWhereWeWork.organizationId, organization.id));
  ...
  describe.concurrent('Onboarding completion: user who skips org selection can access the app', () => {
