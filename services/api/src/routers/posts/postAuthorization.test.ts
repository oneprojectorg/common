...
// Posts attached to a proposal (profiles.type = 'proposal') don't carry their
// own decision permissions — they inherit from the parent decision. The new
// rootProfileId column resolves that at write time, so auth checks against
// proposal-attached posts dispatch on the *decision* gate, not the proposal.
describe.concurrent('proposal-attached post authorization', () => {
  it('rejects an outsider from reacting to a proposal-attached post', async ({
    task,
    onTestFinished,
  }) => {
    const testData = new TestDecisionsDataManager(task.id, onTestFinished);
    ...
    const ownerCaller = await createAuthenticatedCaller(setup.userEmail);
    const proposalPost = await ownerCaller.posts.createPost({
    ...
    // Verify the gate landed on the decision profile, not the proposal.
    const [gateRow] = await db
      .select({ rootProfileId: posts.rootProfileId })
      .from(posts)
      .where(eq(posts.id, proposalPost.id))
      .limit(1);
    expect(gateRow?.rootProfileId).toBe(instance.profileId);
    ...
    const reactions = await db
      .select({ postId: postReactions.postId })
      .from(postReactions)
      .where(eq(postReactions.postId, proposalPost.id));
    expect(reactions).toHaveLength(0);
  });

  it('allows a decision member to react to a proposal-attached post', async ({
    task,
    onTestFinished,
  }) => {
    const testData = new TestDecisionsDataManager(task.id, onTestFinished);
  ...
  it('rejects an outsider from reading a proposal comment thread', async ({
    task,
    onTestFinished,
  }) => {
    const testData = new TestDecisionsDataManager(task.id, onTestFinished);
    ...
    // getPosts with parentPostId only — the path that previously walked
    // postsToProfiles and missed the actual decision gate.
    await expect(
      outsiderCaller.posts.getPosts({
