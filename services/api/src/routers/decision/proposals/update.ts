...
export const updateProposalRouter = router({
  updateProposal: authenticatedProcedure({
    ...
    .mutation(async ({ ctx, input }) => {
      ...
      // updateProposal enqueues `content/submitted` through the transactional
      // outbox when editing a non-draft proposal, so re-moderation on edits
      // survives an Inngest brownout.
