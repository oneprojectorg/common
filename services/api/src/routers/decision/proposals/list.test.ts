...
describe.concurrent('listProposals: scope all', () => {
  it('bypasses phase scoping to surface proposals not carried into the current phase', async ({
    ...
    const allValid = await caller.decision.listProposals({
      processInstanceId: instanceId,
      scope: 'all',
    });
  ...
  it('still excludes DRAFT proposals under scope all', async ({
    task,
    onTestFinished,
  }) => {
    const testData = new TestDecisionsDataManager(task.id, onTestFinished);
  ...
  it('still excludes soft-deleted proposals under scope all', async ({
    task,
    onTestFinished,
  }) => {
    const testData = new TestDecisionsDataManager(task.id, onTestFinished);
  ...
  it('still hides HIDDEN proposals from non-admin, non-owner viewers under scope all', async ({
    task,
    onTestFinished,
  }) => {
    const testData = new TestDecisionsDataManager(task.id, onTestFinished);
  ...
  it('rejects requests that combine scope="all" with phaseId', async ({
    task,
    onTestFinished,
  }) => {
    const testData = new TestDecisionsDataManager(task.id, onTestFinished);
