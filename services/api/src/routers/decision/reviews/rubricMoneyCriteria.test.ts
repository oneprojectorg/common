...
const rubricTemplate: RubricTemplateSchema = {
  ...
  properties: {
    [DESIGN]: moneyCriterion('Design cost'),
    [CONSTRUCTION]: moneyCriterion('Materials cost'),
    [CONTINGENCY]: moneyCriterion('Contingency'),
    impact: {
      type: 'integer',
      title: 'Impact',
      'x-format': 'dropdown',
...
describe.concurrent('rubric money criteria', () => {
  ...
  it('accepts a submission with an optional money criterion left blank', async (ctx) => {
    const { created, caller } = await seedAssignment(
      ctx.task,
      ctx.onTestFinished,
    );

    // A cleared money input drops the key — this is the shape the form sends.
    const answers = {
      [DESIGN]: validMoneyAnswers[DESIGN],
      [CONSTRUCTION]: validMoneyAnswers[CONSTRUCTION],
      impact: 5,
    };
    ...
    expect(stored?.reviewData).toMatchObject({ answers });
    expect(stored?.reviewData).not.toMatchObject({
      answers: { [CONTINGENCY]: expect.anything() },
    });
  });
