...
// Generated exports stay out of the public buckets above. A proposals CSV
// carries submitter names and a personal data export carries one subject's whole
// record, so a reader must present a signed URL. The name must match
// `EXPORTS_BUCKET` in @op/common. It is spelled out here, the same way `assets`
// and `avatars` are, because `services/db` does not depend on `@op/common`.
