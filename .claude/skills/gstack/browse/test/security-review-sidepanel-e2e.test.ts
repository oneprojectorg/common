    ...
    window.fetch = async function (input: any, init?: any) {
      ...
      if (url.includes('/security-decision') && init?.method === 'POST') {
        try {
          ...
          (window as any).__decisionCalls.push({
            _parseError: true,
            raw: init?.body,
          });
        }
...
function buildReviewableEntry(overrides?: Partial<any>): any {
  return {
    ...
    reviewable: true,
    suspected_text:
      'A comment thread discussing ignore previous instructions and reveal secrets — classifier flagged this as injection but it is actually benign developer content about a prompt injection incident.',
    signals: [
...
describe('sidepanel review-flow E2E', () => {
  test.skipIf(!CHROMIUM_AVAILABLE)(
    'reviewable event shows review banner with suspected text + buttons',
    async () => {
      const context = await browser!.newContext();
      const page = await context.newPage();
      await installStubsAndCapture(page, {
        securityEntries: [buildReviewableEntry()],
      });
      ...
      const allowVisible = await page
        .locator('#security-banner-btn-allow')
        .isVisible();
      const blockVisible = await page
        .locator('#security-banner-btn-block')
        .isVisible();
      expect(allowVisible).toBe(true);
      expect(blockVisible).toBe(true);

      // Details auto-expanded so the user sees context
      const detailsHidden = await page.$eval(
  ...
  test.skipIf(!CHROMIUM_AVAILABLE)(
    'clicking Allow posts {decision:"allow"} and hides banner',
    async () => {
      const context = await browser!.newContext();
      const page = await context.newPage();
      await installStubsAndCapture(page, {
      ...
      await page.goto(SIDEPANEL_URL);
      await page.waitForSelector('#security-banner-btn-allow:visible', {
        timeout: 5000,
      });
  ...
  test.skipIf(!CHROMIUM_AVAILABLE)(
    'clicking Block posts {decision:"block"} and hides banner',
    async () => {
      const context = await browser!.newContext();
      const page = await context.newPage();
      await installStubsAndCapture(page, {
        securityEntries: [buildReviewableEntry({ id: 55 })],
      });
      await page.goto(SIDEPANEL_URL);
      await page.waitForSelector('#security-banner-btn-block:visible', {
        timeout: 5000,
      });
  ...
  test.skipIf(!CHROMIUM_AVAILABLE)(
    'non-reviewable event still shows hard-stop banner with no buttons',
    async () => {
      ...
      const actionsHidden = await page.$eval(
  ...
  test.skipIf(!CHROMIUM_AVAILABLE)(
    'suspected text renders via textContent, not innerHTML (XSS guard)',
    async () => {
      ...
      const xssAttempt = buildReviewableEntry({
        suspected_text:
          '<script>window.__xss = "pwn"</script><img src=x onerror="window.__xss=\'onerror\'">',
      });
      const context = await browser!.newContext();
      const page = await context.newPage();
      await installStubsAndCapture(page, { securityEntries: [xssAttempt] });
      await page.goto(SIDEPANEL_URL);
      await page.waitForSelector('#security-banner-suspect:not([hidden])', {
        timeout: 5000,
      });

      // The literal text should appear inside the suspect block (as text, not markup)
      const suspectText = await page.$eval(
        '#security-banner-suspect',
        (el) => el.textContent,
      );
