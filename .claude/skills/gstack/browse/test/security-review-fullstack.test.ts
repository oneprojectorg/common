...
const CLASSIFIER_CACHE = path.join(
  os.homedir(),
  '.gstack',
  'models',
  'testsavant-small',
...
const CLASSIFIER_READY = (() => {
  try {
    ...
    return (
      fs.existsSync(path.join(CLASSIFIER_CACHE, 'tokenizer.json')) &&
...
describe('review-flow full-stack E2E', () => {
  test.skipIf(!CLASSIFIER_READY)(
    ...
    async () => {
      const attemptsDir = fs.mkdtempSync(
        path.join(os.tmpdir(), 'attempts-allow-'),
      );
      try {
        ...
        expect(String(reviewable.suspected_text ?? '')).toContain(
          'IGNORE ALL PREVIOUS',
        );
        ...
        const overrodeLog = attempts.find(
          (a) =>
            a.verdict === 'user_overrode' && a.layer === 'testsavant_content',
        );
        ...
        expect(JSON.stringify(overrodeLog)).not.toContain(
          'IGNORE ALL PREVIOUS',
        );
  ...
  test.skipIf(!CLASSIFIER_READY)(
    ...
    async () => {
      const attemptsDir = fs.mkdtempSync(
        path.join(os.tmpdir(), 'attempts-block-'),
      );
      try {
        await startStack('tool_result_injection', attemptsDir);
        ...
        const followupAttempted = (finalChatData.entries ?? []).some(
          (entry: any) =>
            entry.type === 'tool_use' &&
            String(entry.input ?? '').includes(
              'post-block-followup.example.com',
            ),
        );
