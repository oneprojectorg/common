...
describe('writeToInbox', () => {
  ...
  test('atomic write — final file exists, no .tmp left', () => {
    ...
    const jsonFiles = files.filter(
      (f) => f.endsWith('-observation.json') && !f.startsWith('.'),
    );
...
describe('describeToolCall', () => {
  ...
  test('goto → plain English with URL', () => {
    const result = describeToolCall('Bash', {
      command: '$B goto https://example.com',
    });
  ...
  test('url → checking current URL', () => {
    expect(describeToolCall('Bash', { command: '$B url' })).toBe(
  ...
  test('back/forward/reload → plain English', () => {
    ...
    expect(describeToolCall('Bash', { command: '$B forward' })).toBe(
      'Going forward',
    );
    expect(describeToolCall('Bash', { command: '$B reload' })).toBe(
      'Reloading the page',
    );
  });
  ...
  test('snapshot -D → checking what changed', () => {
    expect(describeToolCall('Bash', { command: '$B snapshot -D' })).toBe(
      'Checking what changed',
    );
  });
  ...
  test('click → clicking element', () => {
    expect(describeToolCall('Bash', { command: '$B click @e3' })).toBe(
      'Clicking @e3',
    );
  });
  ...
  test('fill → typing into element', () => {
    expect(
      describeToolCall('Bash', { command: '$B fill @e4 "hello world"' }),
    ).toBe('Typing ""hello world"" into @e4');
  });

  test('scroll with selector → scrolling to element', () => {
    expect(describeToolCall('Bash', { command: '$B scroll .footer' })).toBe(
      'Scrolling to .footer',
    );
  });

  test('scroll without args → scrolling down', () => {
    expect(describeToolCall('Bash', { command: '$B scroll' })).toBe(
      'Scrolling down',
    );
  });
  ...
  test('text → reading page text', () => {
    expect(describeToolCall('Bash', { command: '$B text' })).toBe(
      'Reading page text',
    );
  });
  ...
  test('html without selector → reading full page HTML', () => {
    expect(describeToolCall('Bash', { command: '$B html' })).toBe(
      'Reading full page HTML',
    );
  });

  test('links → finding all links', () => {
    expect(describeToolCall('Bash', { command: '$B links' })).toBe(
  ...
  test('style → changing CSS', () => {
    expect(
      describeToolCall('Bash', { command: '$B style .header color red' }),
    ).toBe('Changing CSS: .header color red');
  });
  ...
  test('screenshot → saving screenshot', () => {
    expect(
      describeToolCall('Bash', { command: '$B screenshot /tmp/shot.png' }),
    ).toBe('Saving screenshot to /tmp/shot.png');
  });

  test('screenshot without path', () => {
    expect(describeToolCall('Bash', { command: '$B screenshot' })).toBe(
      'Saving screenshot',
    );
  });
  ...
  test('Read tool → reading file', () => {
    expect(
      describeToolCall('Read', { file_path: '/Users/foo/project/src/app.ts' }),
    ).toBe('Reading ~/project/src/app.ts');
  });
  ...
  test('Edit tool → editing file', () => {
    expect(
      describeToolCall('Edit', { file_path: '/Users/foo/src/main.ts' }),
    ).toBe('Editing ~/src/main.ts');
  });
  ...
  test('unknown browse command → generic description', () => {
    expect(
      describeToolCall('Bash', { command: '$B newtab https://foo.com' }),
    ).toContain('newtab');
  });

  test('non-browse bash → shortened command', () => {
    expect(describeToolCall('Bash', { command: 'echo hello' })).toBe(
      'echo hello',
    );
  });
  ...
  test('full browse binary path recognized', () => {
    const result = describeToolCall('Bash', {
      command:
        '/Users/garrytan/.claude/skills/gstack/browse/dist/browse goto https://example.com',
    });
    expect(result).toBe('Opening https://example.com');
  });
...
describe('per-tab agent concurrency', () => {
  ...
  const agentSrc = fs.readFileSync(
    path.join(__dirname, '..', 'src', 'sidebar-agent.ts'),
    'utf-8',
  );
...
describe('BROWSE_TAB tab pinning (cross-tab isolation)', () => {
  ...
  test('handleCommandInternal accepts tabId from request body', () => {
    const handleFn = serverSrc.slice(
      ...
      serverSrc.indexOf(
        '\n/** HTTP wrapper',
        serverSrc.indexOf('async function handleCommandInternal(') + 1,
      ) > 0
        ? serverSrc.indexOf(
            '\n/** HTTP wrapper',
            serverSrc.indexOf('async function handleCommandInternal(') + 1,
        ...
        : serverSrc.indexOf(
            '\nasync function ',
            serverSrc.indexOf('async function handleCommandInternal(') + 200,
          ),
    );
  ...
  test('handleCommandInternal restores active tab after command (success path)', () => {
    ...
    const restoreCount = (handleFn.match(/switchTab\(savedTabId/g) || [])
      .length;
