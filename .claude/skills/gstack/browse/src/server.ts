...
const ANALYSIS_WORDS =
  /\b(what|why|how|explain|describe|summarize|analyze|compare|review|read\b.*\b(and|then)|tell\s*me|find.*bugs?|check.*for|assess|evaluate|report)\b/i;
const ACTION_PATTERNS =
  /^(go\s*to|open|navigate|click|tap|press|fill|type|enter|scroll|screenshot|snap|reload|refresh|back|forward|close|submit|select|toggle|expand|collapse|dismiss|accept|upload|download|focus|hover|cleanup|clean\s*up)\b/i;
const ACTION_ANYWHERE =
  /\b(go\s*to|click|tap|fill\s*(in|out)?|type\s*in|navigate\s*to|open\s*(the|this|that)?|take\s*a?\s*screenshot|scroll\s*(down|up|to)|reload|refresh|submit|press\s*(the|enter|button))\b/i;
...
function shortenPath(str: string): string {
  return str
    .replace(
      new RegExp(BROWSE_BIN.replace(/[.*+?^${}()|[\]\\]/g, '\\$&'), 'g'),
      '$B',
    )
    .replace(/\/Users\/[^/]+/g, '~')
...
function addChatEntry(entry: Omit<ChatEntry, 'id'>, tabId?: number): ChatEntry {
  const targetTab =
    tabId ?? agentTabId ?? browserManager?.getActiveTabId?.() ?? 0;
...
function loadSession(): SidebarSession | null {
  try {
    ...
    try {
      ...
      if (discarded > 0)
        console.warn(
          `[browse] Discarding ${discarded} corrupted chat entries during load`,
        );
      chatBuffer = parsed.filter(Boolean);
      chatNextId =
        chatBuffer.length > 0
          ? Math.max(...chatBuffer.map((e) => e.id)) + 1
          : 0;
    } catch (err: any) {
      if (err.code !== 'ENOENT')
  ...
  try {
    ...
    if (fs.existsSync(worktreeDir)) {
      ...
      try {
        ...
        console.warn(
          '[browse] Failed to clean stale worktree dir:',
          err.message,
        );
      }
    }
    ...
    if (result.exitCode !== 0) {
      console.log(
        `[browse] Worktree creation failed: ${result.stderr.toString().trim()}`,
      );
      return null;
    }
...
function removeWorktree(worktreePath: string | null): void {
  ...
  try {
    ...
    try {
      ...
      console.warn(
        '[browse] Failed to remove worktree dir:',
        worktreePath,
        err.message,
      );
    }
...
function createSession(): SidebarSession {
  ...
  fs.writeFileSync(
    path.join(SESSIONS_DIR, 'active.json'),
    JSON.stringify({ id }),
    { mode: 0o600 },
  );
  chatBuffer = [];
...
function listSessions(): Array<SidebarSession & { chatLines: number }> {
  try {
    const dirs = fs
      ...
      .filter((d) => d !== 'active.json');
    return dirs
      .map((d) => {
        try {
          const session = JSON.parse(
            fs.readFileSync(
              path.join(SESSIONS_DIR, d, 'session.json'),
              'utf-8',
            ),
          );
          let chatLines = 0;
...
function spawnClaude(
  ...
  console.log(
    `[browse] Sidebar model: ${model} for "${userMessage.slice(0, 60)}"`,
  );
  ...
  agentHealthInterval = setInterval(() => {
    ...
    for (const [tid, state] of tabAgents) {
      if (
        ...
        state.status = 'hung';
        console.log(
          `[browse] Sidebar agent for tab ${tid} hung (>${AGENT_TIMEOUT_MS / 1000}s)`,
        );
      }
    }
    // Legacy global check
  ...
  if (
    ...
    if (
      !browserManager.checkTabAccess(targetTab, tokenInfo.clientId, {
        ...
        ownOnly: tokenInfo.tabPolicy === 'own-only',
      })
    ) {
      return {
        status: 403,
...
async function shutdown(exitCode: number = 0) {
  ...
  try {
    ...
    spawnSync('pkill', ['-f', 'sidebar-agent\\.ts'], {
      stdio: 'ignore',
      timeout: 3000,
    });
  } catch (err: any) {
    ...
    async (req: Request): Promise<Response> => {
        ...
        return new Response(
          JSON.stringify({
            ...
            session: sidebarSession
              ? { id: sidebarSession.id, name: sidebarSession.name }
              : null,
      ...
      if (url.pathname === '/sidebar-tabs') {
        ...
        try {
          ...
          return new Response(
            JSON.stringify({ tabs: [], error: err.message }),
            {
              status: 200,
              headers: {
                'Content-Type': 'application/json',
      ...
      if (url.pathname === '/sidebar-chat') {
        ...
        const tabAgentStatus =
          tabId !== null ? getTabAgentStatus(tabId) : agentStatus;
      ...
      if (url.pathname === '/sidebar-command' && req.method === 'POST') {
        ...
        if (tabState.status === 'idle') {
          ...
          tabState.queue.push({
            message: msg,
            ts,
            extensionUrl: sanitizedExtUrl,
          });
          return new Response(
            JSON.stringify({
              ok: true,
              queued: true,
              position: tabState.queue.length,
            }),
            {
              status: 200,
              headers: { 'Content-Type': 'application/json' },
      ...
      if (url.pathname === '/sidebar-chat/clear' && req.method === 'POST') {
        ...
        if (sidebarSession) {
          ...
          try {
            fs.writeFileSync(chatFile, '', { mode: 0o600 });
          } catch (err: any) {
            if (err?.code !== 'ENOENT')
      ...
      if (url.pathname === '/sidebar-agent/stop' && req.method === 'POST') {
        ...
        return new Response(
          JSON.stringify({ ok: true, queuedMessages: messageQueue.length }),
          {
            status: 200,
            headers: { 'Content-Type': 'application/json' },
          },
      ...
      if (url.pathname === '/sidebar-queue/dismiss' && req.method === 'POST') {
        ...
        return new Response(
          JSON.stringify({ ok: true, queueLength: messageQueue.length }),
          {
            status: 200,
            headers: { 'Content-Type': 'application/json' },
          },
      ...
      if (url.pathname === '/sidebar-session/new' && req.method === 'POST') {
        ...
        sidebarSession = createSession();
        return new Response(
          JSON.stringify({ ok: true, session: sidebarSession }),
          {
            status: 200,
            headers: { 'Content-Type': 'application/json' },
          },
      ...
      if (url.pathname === '/sidebar-agent/event' && req.method === 'POST') {
        ...
        if (body.type === 'agent_done' || body.type === 'agent_error') {
          ...
          const anyActive = [...tabAgents.values()].some(
