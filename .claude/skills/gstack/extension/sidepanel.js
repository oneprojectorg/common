...
function showSecurityBanner(event) {
  ...
  // Title + subtitle
  if (title)
    title.textContent = reviewable
      ? 'Review suspected injection'
      : 'Session terminated';
  if (subtitle) {
  ...
  if (suspectEl && suspectLabel) {
    if (
      reviewable &&
      typeof event.suspected_text === 'string' &&
      event.suspected_text.length > 0
    ) {
  ...
  if (actions && btnAllow && btnBlock) {
    ...
    if (reviewable) {
      ...
      freshAllow.addEventListener('click', () =>
        postSecurityDecision(tabId, 'allow'),
      );
      freshBlock.addEventListener('click', () =>
        postSecurityDecision(tabId, 'block'),
      );
    }
  }
  ...
  if (expandBtn && details) {
    ...
    if (chevron)
      chevron.style.transform = reviewable ? 'rotate(180deg)' : 'rotate(0deg)';
  }
...
const SHIELD_LABELS = {
  ...
  degraded: {
    label: 'SEC',
    aria: 'Security status: degraded (some layers offline)',
  },
  inactive: {
    label: 'SEC',
    aria: 'Security status: inactive (architectural controls only)',
  },
};
function updateSecurityShield(securityState) {
  ...
  if (securityState.layers) {
    const parts = Object.entries(securityState.layers).map(
      ([k, v]) => `${k}:${v}`,
    );
...
document.addEventListener('DOMContentLoaded', () => {
  ...
  if (expandBtn) {
    expandBtn.addEventListener('click', () => {
      const details = document.getElementById('security-banner-details');
      const chevron =
        banner && banner.querySelector('.security-banner-chevron');
      if (!details) return;
...
async function pollChat() {
  ...
  try {
    ...
    const tabParam =
      sidebarActiveTabId !== null ? `&tabId=${sidebarActiveTabId}` : '';
    const resp = await fetch(
      `${serverUrl}/sidebar-chat?after=${chatLineCount}${tabParam}`,
      {
        headers: authHeaders(),
    ...
    if (!resp.ok) {
      console.warn(
        `[gstack sidebar] Chat poll failed: ${resp.status} ${resp.statusText}`,
      );
      return;
    }
    ...
    if (
      data.activeTabId !== undefined &&
      data.activeTabId !== sidebarActiveTabId
    ) {
...
async function stopAgent() {
  if (!serverUrl) return;
  try {
    const resp = await fetch(`${serverUrl}/sidebar-agent/stop`, {
      method: 'POST',
      headers: authHeaders(),
    });
    if (!resp.ok)
      console.warn(`[gstack sidebar] Stop agent failed: ${resp.status}`);
  } catch (err) {
...
async function pollTabs() {
  ...
  try {
    ...
    try {
      ...
      console.debug(
        '[gstack sidebar] Failed to get active tab URL:',
        err.message,
      );
    }
    ...
    if (!resp.ok) {
      console.warn(
        `[gstack sidebar] Tab poll failed: ${resp.status} ${resp.statusText}`,
      );
      return;
    }
    const data = await resp.json();
...
async function switchBrowserTab(tabId) {
  ...
  try {
    ...
    console.error(
      '[gstack sidebar] Failed to switch browser tab:',
      err.message,
    );
  }
}
...
document.getElementById('clear-chat').addEventListener('click', async () => {
  if (!serverUrl) return;
  try {
    const resp = await fetch(`${serverUrl}/sidebar-chat/clear`, {
      method: 'POST',
      headers: authHeaders(),
    });
    if (!resp.ok)
      console.warn(`[gstack sidebar] Clear chat failed: ${resp.status}`);
  } catch (err) {
...
async function runCleanup(...buttons) {
  ...
  try {
    ...
    if (resp.ok) {
      addChatEntry({
        type: 'notification',
        message: 'Cleaning up page (agent is analyzing...)',
      });
    } else {
      addChatEntry({
        type: 'notification',
...
chrome.runtime.onMessage.addListener((msg) => {
  ...
  if (msg.type === 'browserTabActivated') {
    ...
    if (serverUrl && serverToken) {
      fetch(
        `${serverUrl}/sidebar-tabs?activeUrl=${encodeURIComponent(msg.url || '')}`,
        {
          headers: authHeaders(),
          signal: AbortSignal.timeout(2000),
        },
