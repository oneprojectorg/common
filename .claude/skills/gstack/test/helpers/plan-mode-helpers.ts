...
export async function runPlanModeSkillTest(opts: {
  ...
  const { skillName, firstAnswerSubstring, omitPlanModeReminder, maxTurns } =
    opts;
        ...
        if (toolName === 'AskUserQuestion') {
          ...
          const matched = isFirst
            ? q.options.find((o) =>
                o.label
                  .toLowerCase()
                  .includes(firstAnswerSubstring.toLowerCase()),
              )
            : undefined;
 ...
 */
export function assertNotHandshakeShape(aq: {
  input: Record<string, unknown>;
}): void {
