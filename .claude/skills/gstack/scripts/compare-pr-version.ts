...
console.log(
  `  Queue (${(parsed.claimed ?? []).length} open PRs claiming versions):`,
);
...
if (cmp(pPR, pNext) >= 0) {
  console.log(
    `✓ ${tag} claims v${prVersion} — slot is free (next would be v${nextSlot}).`,
  );
  process.exit(0);
}
...
console.log(
  `::error::VERSION drift: ${tag} claims v${prVersion} but the queue has moved — next free slot is v${nextSlot}.`,
);
console.log(
  `::error::Rerun /ship from the feature branch to reconcile. /ship's ALREADY_BUMPED branch handles this atomically (VERSION, package.json, CHANGELOG, PR title).`,
);
process.exit(1);
