...
> **IMPORTANT:** This repo is pnpm-only. Always run as `pnpm dlx shadcn@latest`. Run from `packages/ui-next` (or `pnpm w:ui-next ...`) since that is the workspace with `components.json`.
...
```bash
pnpm dlx shadcn@latest apply [preset] [options]
```
...
# Works with URLs too.
pnpm dlx shadcn@latest add https://api.npoint.io/abc123 --dry-run
...
> **`pnpm dlx shadcn@latest add --dry-run` vs `pnpm dlx shadcn@latest view`:** Prefer `pnpm dlx shadcn@latest add --dry-run/--diff/--view` over `pnpm dlx shadcn@latest view` when the user wants to preview changes to their project. `pnpm dlx shadcn@latest view` only shows raw registry metadata. `pnpm dlx shadcn@latest add --dry-run` shows exactly what would happen in the user's project: resolved file paths, diffs against existing files, and CSS updates. Use `pnpm dlx shadcn@latest view` only when the user wants to browse registry info without a project context.
...
Do not use this command. Use `pnpm dlx shadcn@latest add --diff` instead.
...
The `info` output includes a **Links** section with templated URLs for component docs, source, and examples. For resolved URLs, use `pnpm dlx shadcn@latest docs <component>` instead.
...
> Use `pnpm dlx shadcn@latest apply --preset <code>` when overwriting an existing project's preset.

## Switching Presets

Ask the user first: **overwrite**, **merge**, or **skip** existing components?

- **Overwrite / Re-install** → `pnpm dlx shadcn@latest apply --preset <code>`. Overwrites all detected component files with the new preset styles. Use when the user hasn't customized components.
- **Merge** → `pnpm dlx shadcn@latest init --preset <code> --force --no-reinstall`, then run `pnpm dlx shadcn@latest info` to get the list of installed components and use the [smart merge workflow](./SKILL.md#updating-components) to update them one by one, preserving local changes. Use when the user has customized components.
