...
export const UserAvatarMenu = ({ className }: { className?: string }) => {
  ...
  const avatarButton = (
    <Button
      unstyled
      className={cn('relative min-h-11 min-w-11 sm:min-h-0 sm:min-w-0', className)}
    ...
    <>
      ...
      <header className="flex h-auto w-full items-center justify-between px-4 py-2 sm:hidden">
        {!isMobileSearchExpanded && (
          <div className="flex items-center gap-3">
            <SidebarTrigger className="min-h-11 min-w-11 sm:min-h-0 sm:min-w-0 sm:p-1" size="small" />
            <Link href="/" className="flex gap-1">
              <CommonLogo />
            </Link>
          </div>
