...
import type { Audience, ProcessDraft, ProcessType, ShapeKey } from './types';
...
export function CreateProcessWizard({
  ...
  return (
    <div className="flex h-dvh flex-col bg-muted">
      ...
      <div className="relative flex min-h-0 flex-1 flex-col overflow-y-auto">
        ...
        <div
          className={cn(
            ...
            step === 1 ? 'flex-1' : 'pt-8 pb-12',
            step === 4 ? 'max-w-xl' : 'max-w-lg',
          )}
        >
          ...
          {step === TOTAL_STEPS ? (
            <NameAccessStep
              ...
              submissionsPrivate={submissionsPrivate}
              onSubmissionsPrivateChange={setSubmissionsPrivate}
            />
          ) : null}
        </div>
      </div>
      ...
      {step > 1 ? (
        <FooterBar position="static" className="relative">
          {/* Mobile: a full-width line on the footer's top edge. Only one of the
            two progress bars is ever in the accessibility tree — `hidden`
            removes the other outright. */}
          <Progress
            value={progress}
          ...
          <FooterBarCenter className="hidden gap-3 sm:flex">
            <Progress
              value={progress}
              aria-label={stepLabel}
