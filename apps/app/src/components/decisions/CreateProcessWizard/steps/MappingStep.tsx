    ...
    <div className="flex flex-col gap-6">
      ...
      >
        {/* The rail, inset to the centre of a node — 11px down, per the margin
            below — so it runs between the first and last rather than past
            them. Behind the nodes by paint order; a negative z-index put it
            behind the page background instead. */}
        {pieces.length > 1 ? (
          <span
            aria-hidden
        ...
        {pieces.map((piece, index) => (
          <PieceRow key={`${piece.name}-${index}`} piece={piece} step={index} />
        ))}
      </Accordion>
    </div>
  );
...
function PieceRow({ piece, step }: { piece: ProcessPiece; step: number }) {
  ...
  return (
    <AccordionItem
      value={step}
      render={<li />}
      className="group/item flex items-start gap-4 border-b-0"
    >
      ...
      <div className="min-w-0 flex-1 overflow-hidden rounded-lg border bg-background">
        <AccordionTrigger
          aria-label={t('Step {step}: {name}', { step: step + 1, name: title })}
          className="w-full gap-4 rounded-none border-0 px-5 py-3.5 hover:bg-muted/40 hover:no-underline **:data-[slot=accordion-trigger-icon]:hidden"
        >
