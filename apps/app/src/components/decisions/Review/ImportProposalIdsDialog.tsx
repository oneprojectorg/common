...
/**
 * Paste-a-spreadsheet shortcut for `AssignProposalsDialog`, stacked on top of
 * it: admins triage in Sheets and arrive with 100+ proposal IDs, which is not a
 * checkbox job. Import only adds to the parent's selection — the admin still
 * reviews the rows and presses assign, so nothing here mutates.
 */
export function ImportProposalIdsDialog({
  ...
  /** Proposals with no blocker for the selected reviewer — the importable set. */
  assignableIds: ReadonlySet<string>;
  /** Blocks the trigger while the summary would be a promise we can't keep. */
  disabled?: boolean;
