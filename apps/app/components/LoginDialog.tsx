...
const LoginDialog = ({
  ...
  const { data: user, refetch: refetchUser, isFetching: isRefetchingUser } = useAuthUser({
  ...
  return (
    <Modal
      ...
      className="w-auto xs:w-96"
      wrapperClassName="backglow after:opacity-60"
    >
      <Dialog>

        <div className="flex flex-col items-center justify-center">
          <div className="aspect-square w-12 rounded-lg p-2">
            <Image src="/op.png" alt="OP" width={48} height={48} />
          </div>
          <DialogHeader className="mt-4 text-2xl">
            {user?.error?.name === 'AuthRetryableFetchError'
              ? 'Connection Issue'
              : (() => {
                  if (login.isError || error || tokenError) {
                    if (combinedError?.includes('invite') || combinedError?.includes('waitlist')) {
                      return 'Stay tuned!';
                    }
                ...
                if (combinedError || tokenError) {
                  return (
                    <span className={cn(tokenError && 'text-red-500')}>
                      {combinedError || tokenError || 'There was an error signing you in.'}
                    </span>
                  );
                }
        ...
        {user?.error?.name !== 'AuthRetryableFetchError' && !(login.isError || !!combinedError) && (
                ...
                >
                  <TextField
                    ...
                    onChange={(val) => {
                      ...
                      setEmailIsValid(emailParser.safeParse(locEmail).success);
                      setEmail(locEmail);
                    }}
                  />
                </Form>
                  ...
                  {user?.error?.name === 'AuthRetryableFetchError'
                    ...
                    : (
                        <Button
                          ...
                          className="flex w-full items-center justify-center"
                          isDisabled={
                            !emailIsValid || login.isFetching || (!!token && token.length !== 6)
                          }
                          onPress={async () => {
                            if (!loginSuccess) {
                              void login.refetch().then(({ data }) => {
