...
Run the `implement-task` skill (shipped by the `devtools` plugin in
the common-agent-toolkit marketplace, installed into the container
image). Pass `TASK_ID={{TASK_ID}}`. The skill owns:
